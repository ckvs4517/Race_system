...
function roundMatchesView(tournament, round, roundIndex, scoringEnabled, correctionEnabled, seedNames, isSwiss, arenaCount) {
  const entries = round.matches.map((match, matchIndex) => ({ match, matchIndex }));
  if (arenaCount === 1) return scoreGroupedMatchesView(tournament, round, roundIndex, entries, scoringEnabled, correctionEnabled, seedNames, isSwiss);
  ...
  return `<div class="battle-stations">${stations.map((stationEntries, stationIndex) => `<section class="battle-station"><div class="battle-station-title"><span>戰鬥台 ${stationIndex + 1}</span><i>${stationEntries.length ? `${stationEntries.length} 場對戰` : '本輪待命'}</i></div>${stationEntries.length ? scoreGroupedMatchesView(tournament, round, roundIndex, stationEntries, scoringEnabled, correctionEnabled, seedNames, isSwiss) : '<div class="battle-station-empty">本輪沒有分配對戰</div>'}</section>`).join('')}</div>`;
}

function scoreGroupedMatchesView(tournament, round, roundIndex, entries, scoringEnabled, correctionEnabled, seedNames, isSwiss) {
  if (!isSwiss || roundIndex === 0 || (round.phase || 'preliminary') !== 'preliminary') return `<div class="station-match-list">${entries.map(({ match, matchIndex }) => matchCard(match, roundIndex, matchIndex, scoringEnabled, correctionEnabled, seedNames, round.seedReason, isSwiss, tournament.status)).join('')}</div>`;
  ...
  return [...groups].map(([label, matches]) => `<section class="swiss-score-group"><div class="swiss-score-group-title"><span>${escapeText(label)}</span><i>${matches.length} 場對戰</i></div><div class="swiss-score-group-matches">${matches.map(({ match, matchIndex }) => matchCard(match, roundIndex, matchIndex, scoringEnabled, correctionEnabled, seedNames, round.seedReason, isSwiss, tournament.status)).join('')}</div></section>`).join('');
}
...
function matchCard(match, roundIndex, matchIndex, scoringEnabled, correctionEnabled, seedNames, seedReason, isSwiss, tournamentStatus) {
